...
This is a Jira ticket number (e.g. `ABC-4221`).
...
Construct the full ticket URL: `${JIRA_URL}/browse/$ARGUMENTS`

Fetch the ticket page to extract the title and description:

```bash
curl -s "${JIRA_URL}/browse/$ARGUMENTS"
```

If you cannot extract meaningful ticket data, try fetching via the Jira REST API:

```bash
curl -s "${JIRA_URL}/rest/api/2/issue/$ARGUMENTS?fields=summary,description,issuetype,priority,labels,components,acceptance" \
...
Based on the ticket requirements, thoroughly explore the codebase:
...
Be specific. Reference real file paths, real function names, real patterns from the codebase. Do not be generic — every suggestion must be grounded in what actually exists in this repo.
